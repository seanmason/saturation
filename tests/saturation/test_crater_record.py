...
from saturation.rim_erasure_calculators import (
    get_rim_erasure_calculator, RadiusRatioConditionalRimOverlapRimErasureCalculator,
)
...
def test_add_in_study_region():
    ...
    record = CraterRecord(
        r_stat=10.0,
        rim_erasure_calculator=RadiusRatioConditionalRimOverlapRimErasureCalculator(3.0, 1.0),
...
def test_crater_radius_ratio_respected():
    # Arrange
    # For a new crater to affect an old crater, (new crater radius) > (old crater radius) / ratio
    ...
    record = CraterRecord(
        r_stat=10,
        rim_erasure_calculator=RadiusRatioConditionalRimOverlapRimErasureCalculator(2.0, 1.0),
        initial_rim_state_calculator=CircumferenceInitialRimStateCalculator(),
        mrp=0.9,
        rmult=1.0,
        study_region_size=1000,
...
def test_get_nnd_after_two_craters():
    # Arrange
    crater1 = _create_crater(id=1, x=100.0, y=100.0, radius=10.0)
    crater2 = _create_crater(id=2, x=100.0, y=150.0, radius=10.0)
    record = CraterRecord(
    ...
    # Assert
    assert r1[0] == crater2.id
    assert r2[0] == crater1.id
    assert r1[1] == r2[1] == 50
