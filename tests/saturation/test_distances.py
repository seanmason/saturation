...
def test_get_mean_nearest_neighbor_distance_random_adds_and_deletes():
    ...
    for n in range(N_REPEATS):
        ...
        craters = [
            (Crater(id=x + 1, x=point[0], y=point[1], radius=10.0), np.random.rand() < P_TRACKED)
        ...
        c2c_dists = [
            min((_get_distance(x.x, x.y, crater.x, crater.y)
                 for x, _ in craters if x not in to_remove and x != crater))
